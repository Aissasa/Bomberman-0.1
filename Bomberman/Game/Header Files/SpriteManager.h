...
public:
	...
	void addToRender(RenderableSpriteType_t type, const Sprite_t* sprite, Coord2D pos);
...
private:
	...
	Renderable_Sprite_Vect_t mPlayerRenderableSpritesVect; // note replace render enemies and player with render char where u pass an enum
	Renderable_Sprite_Vect_t mBombsRenderableSpritesVect;
	Renderable_Sprite_Vect_t mBombsAERenderableSpritesVect;
	Renderable_Sprite_Vect_t mFadingSoftBlocksRenderableSpritesVect;
	...
	void renderBasicMap();
	void renderItems();
	void renderObject(Renderable_Sprite_Vect_t& vect, SpriteSheet_t& spriteSheet);
